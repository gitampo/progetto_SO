...
int main() {
    ...
    bullet_args b_args[MAX_BULLETS];  // Array per memorizzare gli argomenti dei thread dei proiettili

    grenade_args g_args[MAX_GRENADES];  // Array per memorizzare gli argomenti dei thread delle granate
    ...
    creaCrocodiles(crocs, startCol, endCol, riverStartRow);
     ...
     croc_args c_args[totalCrocs];

    for (int i = 0; i < totalCrocs; i++) {

        c_args[i] = (croc_args){crocs[i], fileds, riverStartRow};

        pthread_create(&thread_crocs[i], NULL, crocThread, &c_args[i]);
    ...
    while (game_win == 0 && lives > 0) {
        ...
        if (read_from_buffer(fileds, &temp)) {
            ...
            if (temp.type == OBJECT_FROG) {
                ...
                b_args[i_last_bullet] = (bullet_args){bullets[i_last_bullet], fileds};
                ...
                {  
                    ...
                    g_args[i_last_grenades] = (grenade_args){grenades[i_last_grenades], fileds};



                    pthread_create(&thread_grenades[i_last_grenades], NULL, grenadeThread, &g_args[i_last_grenades]);
